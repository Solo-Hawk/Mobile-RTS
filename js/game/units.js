...
class Steerable extends Phaser.GameObjects.Sprite {
  ...
  applySteering(){
    switch(this.moveMode){
      ...
      case gametools.statics.steeringBehaviours.ARRIVAL : this.seek(this.objective.position, 50); break;
  ...
  seek(target, radius, brace){
    ...
    if (distance - brace <= radius){

      this.desiredVelocity.scale(this.maxLinearSpeed * (distance - brace)/(radius))
    ...
    // console.log(this.steering.length() - distance);s
